...
import re, urllib2, time, threading, os, sys

def allfunctions():
    result = '\n'.join(item for item in urllib2.urlopen("http://www.hacker.org/runaway/index.php?name=popey456963&gotolevel=12&password=" + password).read().splitlines() if '<PARAM NAME=FlashVars VALUE="FVterrainString=' in item).split("&")
    ...
    def send(x):
        path = "http://www.hacker.org/runaway/index.php?name=popey456963&gotolevel=12&password=" + password + "&path=" + ''.join(x)
